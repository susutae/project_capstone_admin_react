  ...
  <List filters={listFilters} sort={{ field: "id", order: "DESC" }}>
    <Datagrid rowClick="edit">
      ...
      <FunctionField
        ...
        render={(record) =>
          `${Number(record.discount_percentage || 0).toFixed(2)}%`
        }
      />
...
const CartForm = () => (
  <SimpleForm className="admin-form">
    <ReferenceInput source="user_id" reference="users">
      <SelectInput
        optionText="email"
        helperText="Choose a user, or leave empty and enter a guest session token."
      />
    </ReferenceInput>
...
export const CartList = () => (
  <List filters={listFilters} sort={{ field: "created_at", order: "DESC" }}>
    <Datagrid rowClick="edit">
      <TextField source="id" />
      <ReferenceField source="user_id" reference="users" emptyText="Guest">
...
const CartItemForm = () => (
  <SimpleForm className="admin-form">
    <ReferenceInput source="cart_id" reference="carts">
...
const OrderForm = ({ create = false }) => (
  <SimpleForm className="admin-form">
    ...
    {create && (
      <TextInput source="auth_token" validate={required()} fullWidth />
    )}
    ...
    {create && (
      <NumberInput source="subtotal_amount" min={0} defaultValue={0} />
    )}
    {create && (
      <NumberInput source="discount_amount" min={0} defaultValue={0} />
    )}
...
const OrderItemForm = () => (
  <SimpleForm className="admin-form">
    <ReferenceInput source="order_id" reference="orders">
      <SelectInput optionText="order_code" validate={required()} />
    </ReferenceInput>
    <ReferenceInput source="product_id" reference="products">
      <SelectInput optionText="title" validate={required()} />
    </ReferenceInput>
    ...
    <NumberInput
      source="subtotal"
      min={0}
      helperText="Optional; the API calculates it from price × quantity."
    />
  </SimpleForm>
);
export const OrderItemCreate = () => (
  <Create redirect="orders">
    <OrderItemForm />
  </Create>
);
